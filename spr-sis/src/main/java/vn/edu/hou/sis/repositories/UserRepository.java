...
public interface UserRepository extends JpaRepository<User, Long>{
	...
	@Query("SELECT r.loai from Role r, UserRole ur where r.id = ur.userRoleId and ur.username = ?1")
	public List<String> getUserRoles(String username);
	@Query("SELECT s from SinhVien s, User u where u.username = ?1 and u.id = s.userId")
